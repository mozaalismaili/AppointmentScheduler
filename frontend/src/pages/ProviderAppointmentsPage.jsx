...
import { useProviderAppointments } from '../queries/appointmentQueries';
import AppointmentList from '../components/AppointmentList';

export default function ProviderAppointmentsPage({ providerId }) {
  const [page, setPage] = useState(0);
  const [filters, setFilters] = useState({ status: '', from: '', to: '' });

  const { data, isLoading, error } = useProviderAppointments(providerId, {
    ...filters, page, size: 10, sort: 'appointmentTime,asc'
  });
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-xl font-semibold">Provider Appointments</h1>
